...
OUTPUT_DIR = "controlnet_results_tmp20test"  
...
if __name__ == "__main__":
    try:
        ...
        # 只取前 TEST_NUM 个样本
        test_samples = PROMPT_LIST[:TEST_NUM]
        total_samples = len(test_samples)
        print(f"\n 开始测试前 {total_samples} 个COCO样本（共 {len(PROMPT_LIST)} 个）")
        for idx, prompt_item in enumerate(test_samples):
            print(f"\n--- 进度: {idx+1}/{total_samples} ---")
            run_single_sample(prompt_item, base, pipe_b2, pipe_b3, generator)

        print(f"\n 前 {total_samples} 个样本处理完成！结果保存在 {OUTPUT_DIR} 文件夹")
