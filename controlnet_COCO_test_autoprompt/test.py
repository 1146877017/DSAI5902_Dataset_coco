...
def get_person_token_indices(tokenizer, prompt: str):
    """
    【修复 2】从右至左反向查找 Token，彻底避免 COCO 原始 Caption 中包含 'person' 带来的误匹配
    """
    inputs = tokenizer(
        ...
        if is_cross:
            ...
            if height * width == seq_len:
                for idx, mask in enumerate(self.masks):
                    ...
                    # 【修复 3】改用双线性插值进行下采样，防止小目标在低分辨率层（如 8x8）下直接丢失
...
def run_single_sample(prompt_item, base, pipe_b2, pipe_b3, generator, mode, is_first_mode):
    ...
    if is_first_mode:
        ...
        # 【修复 1】不再重复推理，直接高速拷贝首轮生成的水印结果，节约巨额显卡时间
        src_b1 = f"results_{MODES[0]}/{sample_name}_baseline1.png"
        if os.path.exists(src_b1): shutil.copy(src_b1, b1_path)
    ...
    if is_first_mode:
        ...
        if os.path.exists(src_b2): shutil.copy(src_b2, b2_path)
    ...
    try:
        ...
        del img4
        clear_gpu_memory()
    except Exception as e: print(f" ❌ Method 核心创新方法运行失败: {str(e)}")
    finally:
        pipe_b3.unet.set_attn_processor(AttnProcessor())

    # 返回单条元数据记录，用于在主程序中汇总生成 downstream 定量评测所需的清单
    return {
        "image_id": img_id,
...
if __name__ == "__main__":
    try:
        ...
        # 【学术大加分点】自动落盘清单文件，为下游自动计算 CLIP Score 铺平道路
