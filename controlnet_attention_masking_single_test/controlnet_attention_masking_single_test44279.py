...
if __name__ == "__main__":
    try:
        ...
        SEED = 42      
        PROMPT = "person1: (a middle-aged asian male chef in a white short-sleeve shirt, packing takeout food with a metal container:1.2), person2: (a middle-aged asian male chef in a white shirt, dark blue pants and a dark blue baseball cap, cooking at the stove:1.2), chinese restaurant kitchen, stainless steel equipment, bright lighting, photorealistic, high detail"
        ...
        # ========== 4组对比实验 ==========
        print("\n1/4 Baseline1：纯文本生成")
        img1 = base(prompt=PROMPT, negative_prompt=NEG_PROMPT, generator=generator, num_inference_steps=30).images[0]
        img1.save(f"{OUTPUT_DIR}/{SAMPLE_NAME}_baseline1.png")
        del base, img1
        clear_gpu_memory()

        print("\n2/4 Baseline2：OpenPose约束")
        img2 = pipe_b2(prompt=PROMPT, negative_prompt=NEG_PROMPT, image=pose, controlnet_conditioning_scale=0.7, generator=generator, num_inference_steps=30).images[0]
        img2.save(f"{OUTPUT_DIR}/{SAMPLE_NAME}_baseline2.png")
        del pipe_b2, img2
        clear_gpu_memory()

        print("\n3/4 Baseline3：OpenPose+Depth约束")
        img3 = pipe_b3(prompt=PROMPT, negative_prompt=NEG_PROMPT, image=[pose, depth], controlnet_conditioning_scale=[0.7,0.5], generator=generator, num_inference_steps=30).images[0]
